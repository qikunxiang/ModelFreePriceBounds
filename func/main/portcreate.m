...
%       conf.forward: logical vector indicating which forward contracts are
%                     available (default is all true)
...
if ~isfield(conf, 'forward') || isempty(conf.forward)
    conf.forward = true(conf.n, 1);
end
...
% stocks
I = eye(n);
port.aff.C = [zeros(n, 1), I(:, conf.forward), ...
    zeros(n, m - 1 - sum(conf.forward))];
...
if isfield(conf, 'call')
    call_A = repelem(I, cellfun(@length, conf.call), 1);
...
% vanilla put options
if isfield(conf, 'put')
    put_A = repelem(-I, cellfun(@length, conf.put), 1);
...
hp_num = length(hp_b);
hp_C = [zeros(hp_num, 1 + sum(conf.forward)), eye(hp_num), ...
    zeros(hp_num, m - 1 - sum(conf.forward) - hp_num)];
...
if isfield(conf, 'cmax') || isfield(conf, 'cmin') ...
    ...
    if isfield(conf, 'cmax')
        ...
        for i = 1:cmax_num
            ...
            cmax_grp{i} = repelem((grp_index_max + (1:strike_num))', ...
                s_num + 1, 1);
            cmax_C{i} = [zeros(strike_num, ...
                1 + sum(conf.forward) + hp_num), ...
                zeros(strike_num, opt_index_max), eye(strike_num), ...
                zeros(strike_num, m - 1 - sum(conf.forward) - hp_num ...
                - opt_index_max - strike_num)];
    ...
    if isfield(conf, 'cmin')
        ...
        for i = 1:cmin_num
            ...
            cmin_C{i} = repelem([zeros(strike_num, ...
                1 + sum(conf.forward) + hp_num), ...
                zeros(strike_num, opt_index_max), eye(strike_num), ...
                zeros(strike_num, m - 1 - sum(conf.forward) - hp_num ...
                - opt_index_max - strike_num)], 2, 1) ...
    ...
    if isfield(conf, 'pmax')
        ...
        for i = 1:pmax_num
            ...
            pmax_grp{i} = repelem((grp_index_max ...
                + (1:2 * strike_num))', ...
                repmat([s_num + 1; s_num], strike_num, 1), 1);
            pmax_C{i} = repelem([zeros(strike_num, ...
                1 + sum(conf.forward) + hp_num), ...
                zeros(strike_num, opt_index_max), eye(strike_num), ...
                zeros(strike_num, m - 1 - sum(conf.forward) - hp_num ...
                - opt_index_max - strike_num)], 2, 1) ...
