% Script to run Experiment 1 with the ACCP algorithm

load('exp/exp1/exp1.mat');

[port, portlim] = portcreate(conf);

price_bounds(2:end, :) = price_bounds(2:end, :) + [0.005, -0.005];
...
if setting == 1
    prevfile = load('exp/exp1/rst/accp_set2.mat');
elseif setting == 2
    prevfile = load('exp/exp1/rst/accp_set3.mat');
elseif setting == 3
    prevfile = load('exp/exp1/rst/accp_set4.mat');
else
    clear prevfile;
end
...
save(sprintf('exp/exp1/rst/accp_set%d.mat', setting));
