...
load('exp/exp4/exp_DIA1.mat');

prevfile = load('exp/exp4/rst/DIA2_ecp_V(100)+B.mat');
...
for repl_id = 1:repl_num
    for lu = 1:2
        ...
        if lu == 1
            fprintf('%d out of %d: upper bound = %.3f, iter = %d\n', ...
                repl_id, repl_num, out_bounds(repl_id, lu), output.iter);
        else
            fprintf('%d out of %d: lower bound = %.3f, iter = %d\n', ...
                repl_id, repl_num, out_bounds(repl_id, lu), output.iter);
        end
    end
end
...
save('exp/exp4/rst/DIA1_ecp_V(100)+B.mat');